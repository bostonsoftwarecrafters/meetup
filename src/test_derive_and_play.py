from dnd_constants import MAGIC_ARROW_STR
...
def test_mock_simple_game(safe_mock_game_g3_setup_teardown):
    game_orig = safe_mock_game_g3_setup_teardown
    game_orig.set_mock_object_location(MAGIC_ARROW_STR,"F5")
    game_orig.do_action_move(EAST,"Go to G4")
    game_orig.do_action_move(EAST,"Go to G5")
    game_orig.do_action_move(NORTH,"Go to F5 pick up arrow")
    game_orig.do_action_move(EAST,"Go to F6")
    game_derived: MockGame = derive_mock_game(game_orig)
    assert game_derived.get_mock_object_location(MAGIC_ARROW_STR) == ["F5"]
